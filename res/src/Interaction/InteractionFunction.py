...
class InteractionFunction(object):
    ...
    def order_interaction_up(self, dialog, doc, ui_id, clicked_id, c4d_lights):
        if ui_id <= clicked_id < ui_id + const.STEP:
            light_id = clicked_id - ui_id - 2

            #Get current light and previous light
            current_light = c4d_lights[light_id]
            previous_light = c4d_lights[light_id-1]
    ...
    def order_interaction_down(self, dialog, doc, ui_id, clicked_id, c4d_lights):
        if ui_id <= clicked_id < ui_id + const.STEP:
            light_id = clicked_id - ui_id - 2

            #Get current light and next light
            current_light = c4d_lights[light_id]
            next_light = c4d_lights[light_id+1]
