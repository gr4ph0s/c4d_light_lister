...
class JsonFunction(object):
    ...
    def isWrittableFolder(self, path):
        ...
        test_path = os.path.join(path, "config_c4d.test")
        can_write = False
    ...
    def get_path(self):
        ...
        temp_dir = os.path.join(tempfile.gettempdir(), "c4d_light_lister")

        if self.isWrittableFolder(plugin_path):
            return os.path.join(plugin_path, "config_c4d.json")

        if self.isWrittableFolder(temp_dir):
            return os.path.join(temp_dir, "config_c4d.json")
