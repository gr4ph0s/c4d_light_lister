...
with localimport('.') as importer:
    ...
    from res.src.Config.ConfigManager import ConfigManager
    from res.src.LightManager import LightManager
    from res.src.Creation.CreationC4D import CreationC4D
    from res.src.Interaction.InteractionC4D import InteractionC4D
...
class mainDialog(c4d.gui.GeDialog):
    def __init__(self):
        ...
        self.creation = CreationC4D()
        self.interaction = InteractionC4D()
        self.c4d_light = []
    ...
    def Message(self, msg, result):
        if msg.GetId() == c4d.BFM_GOTFOCUS:
            ...
            light_data = self.GetVisibleArea(const.GRP_TAB_C4D_SCROLL_LIGHT)
            name_data = self.GetVisibleArea(const.GRP_TAB_C4D_SCROLL_NAME)

            initial_name_y2 = name_data["y2"] - name_data["y1"]
            initial_light_y2 = light_data["y2"] - light_data["y1"]

            self.SetVisibleArea(const.GRP_TAB_C4D_SCROLL_NAME,
                                name_data["x1"], new_value,
                                name_data["x2"], initial_name_y2 + new_value
                                )

            self.SetVisibleArea(const.GRP_TAB_C4D_SCROLL_LIGHT,
                                light_data["x1"], new_value,
                                light_data["x2"], initial_light_y2 + new_value
                                )

        elif msg.GetId() == 1648576083:
            self.creation.refresh_c4d_light(self, self.c4d_light)
    ...
    def CreateLayout(self):
        ...
        self.c4d_light = self.lm.get_c4d_light()
        self.layers = self.lm.get_all_layers()

        self.SetTitle('C4D Light Lister v{0} - gr4ph0s.xyz'.format(str(const.VERSION)))
        self.SetTimer(500)

        self.create_menu()

        self.create_c4d_data()
    ...
    def create_c4d_data(self, refresh=False):
        if refresh:
        ...
        if self.GroupBegin(const.GRP_TAB_C4D_GRP + 3, c4d.BFH_SCALEFIT | c4d.BFV_SCALEFIT, 300, 200, "C4D"):
            self.creation.create_c4d_light(self, self.config.c4dConfig, self.c4d_light, self.layers)
        self.GroupEnd()
    ...
    def Command(self, id, msg):
        if const.LIGHT_LISTER_C4D_START < id < const.LIGHT_LISTER_C4D_END:
            self.event_from_interaction = True

            self.doc.StartUndo()
            self.interaction.c4d_interaction(self, self.doc, id, msg, self.c4d_light, self.layers)
            self.doc.EndUndo()
            c4d.EventAdd()
    ...
    def refreshLightData(self):
        #store existing data
        save_scroll = [
            self.GetVisibleArea(const.GRP_TAB_C4D_SCROLL_NAME),
            self.GetVisibleArea(const.GRP_TAB_C4D_SCROLL_LIGHT)
        ]
        ...
        #redraw all light
        self.create_c4d_data(True)

        #set scroll
        buffer_data = [
            const.GRP_TAB_C4D_SCROLL_NAME,
            const.GRP_TAB_C4D_SCROLL_LIGHT
        ]
...
class UILauncher_Main(c4d.plugins.CommandData):
 ...
 def Execute(self, doc):
    ...
    return self.dialog.Open(dlgtype=c4d.DLG_TYPE_ASYNC, pluginid=const.PLUGIN_ID, defaultw=200, defaulth=150)
...
class C4DLightListerTag(c4d.plugins.TagData):
...
if __name__ == "__main__":
    ...
    c4d.plugins.RegisterCommandPlugin(  id=const.PLUGIN_ID,
                                        str="C4D Light Lister",
                                        help="List all C4D lights in the scenes",
                                        info=0,
                                        dat=UILauncher_Main(), 
                                        icon=bmp)
